...
export const useWordId = routeLoader$(async ({ params }) => {
  const res = await fetch(
    `${apiString}?action=words&ids=${encodeURI(`["${params.id}"]`)}`,
  );
  ...
  return (
    <div class="mt-4">
      <div class="flex flex-col gap-4 px-4 md:p-0 mx-auto max-w-[560px] items-center">
        ...
        <VideoPlayer
          movieImage={word.value.movie_image}
          movie={word.value.movie}
        />
            ...
            <div class="flex flex-col gap-4">
              {word.value.phrases.map((phrase) => {
                return (
                  ...
                  >
                    <summary class="flex items-center text-xl mb-4">
                      {phrase.phrase}
                    </summary>
                    <VideoPlayer
                      movieImage={phrase.movie_image}
                      movie={phrase.movie}
                    />
                  </details>
                );
